...
def create_logger(curdir: str, params: Dict, l_params: Dict, log_params: Dict, train: bool, weights_path=None, run_tag: str = "") -> Tuple:
    ...
    log = Logger(curdir, params, l_params, log_params, train=train, buffer_size=log_params["buffer_size"], weights_file=weights_path, run_tag=run_tag)
...
def main(args) -> None:
    ...
    params, l_params, v_params, log_params = read_params(args.params_path, args.learning_params_path, args.visualizer_params_path, args.logger_params_path)
    ...
    if args.train:
        (
            ...
            scatter_action_dict,
            scatter_reward_dict,
        ) = qmix_nn.create_agent(params, l_params, n_obs, n_actions, True)
        logger, train_log_every = create_logger(curdir, params, l_params, log_params, True, run_tag=run_tag)
        start = datetime.datetime.now()
        agent_nets, mixer = qmix_nn.train(
            env, params, l_params, agent_nets, mixer,
            ...
            train_episodes, train_log_every, agent_lr, mixer_lr, gamma, decay_type, decay,
            epsilon, epsilon_min, args.print_metrics, logger, env_vis,
        )
        end = datetime.datetime.now()
        logger.save_computation_time(end - start)
        print(f"Training time: {end - start}\n")
        logger.save_model(qmix_nn.pack_model(agent_nets, mixer))
    else:
        (
            agent_nets,
            mixer,
            ...
            scatter_action_dict,
            scatter_reward_dict,
        ) = qmix_nn.create_agent(params, l_params, n_obs, n_actions, False)
        ...
        qmix_nn.unpack_model(weights, agent_nets, mixer)
        start = datetime.datetime.now()
        qmix_nn.eval(
            env, params, cluster_dict, cluster_actions_dict, cluster_action_dict, cluster_reward_dict,
            scatter_actions_dict, scatter_action_dict, scatter_reward_dict,
            test_episodes, agent_nets, test_log_every, logger, env_vis,
        )
        end = datetime.datetime.now()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_params_path", type=str, default="agents/QMIXLearningNN/config/learning-params.json", required=False)
    parser.add_argument("--logger_params_path", type=str, default="agents/QMIXLearningNN/config/logger-params.json", required=False)
    parser.add_argument("--qtable_path", type=str, required=False)
